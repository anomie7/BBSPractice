...
<%
	...
	final String insertSQL = "insert into freeboard(id, name, password, email, subject"
							 + ",content, inputdate, masterid, readcount, replynum, step) " 
							 + "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	final String selectMaxrnum = "select max(replynum) from freeboard where masterid = ?";
	...
	try{
		...
		if(step == 1){
			pstmt = conn.prepareStatement(selectMaxrnum);
			pstmt.setInt(1, mid);
			rs = pstmt.executeQuery();
			if(!(rs.next()))
				rnum = 1;
